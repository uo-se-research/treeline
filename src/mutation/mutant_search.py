...
class Candidate(Scorable):
    ...
    def __str__(self):
        if self.parent:
            ...
            pedigree = f" (root)"
        return f"[{self.score():3.2}: {self.reasons} c={self.cost:4_} {self.count_successful}/{self.count_selections}] '{self.root}' {pedigree}"
...
class Search:
    ...
    def __init__(self, gram: gramm.grammar.Grammar, logdir: pathlib.Path):
        ...
        self.gram = gram
        self.logdir = logdir
        self.input_handler = InputHandler(search_config.FUZZ_SERVER, search_config.FUZZ_PORT)
    ...
    def report(self) -> str:
        ...
        report = f"""
            *** Summary of search ***
            ...
            {self.max_cost} highest execution cost encountered
            {self.count_hnb} occurrences new coverage (AFL bucketed criterion)
            {self.count_hnm} occurrences new max count on an edge (AFL mod in TreeLine and PerfFuzz)
            {self.count_hnc} occurrences new max of edges executed (measure of execution cost)
            ---
            ...
            {self.count_stale} stale mutants (duplicated previously generated string)
            {self.count_valid} valid (not duplicate) mutants submitted for execution
            ---
